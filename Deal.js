	...
	this.getMsg = function(msg, callback) {
		...
		switch (msg.from) {
			...
			case 'ChangePassword':
				...
				break;
			case 'Update':
				msgUpdate(msg.data, function(_json) {
					callback(_json);
				})
				break;
			case 'Delete':
...
// 用户数据上传
	function msgUpdate(data, callback) {
		json = {
			from: 'Update',
			msg: MSG_K.FAIL,
			data: '失败',
		...
		client.query('update forumuser set time = ? where userid = ?', [data.time, data.userid], function(err, rows, fields) {
			if (err) {
				json.data = '上传失败';
				callback(json);
				return;
		...
		var str = JSON.stringify(data.CardInfo);
		client.query('update forumuser set CardInfo = ? where userid = ?', [str, data.userid], function(err, rows, fields) {
			if (err) {
				json.data = '上传失败';
				callback(json);
				return;
		...
		var str = JSON.stringify(data.Review);
		client.query('update forumuser set Review = ? where userid = ?', [str, data.userid], function(err, rows, fields) {
			if (err) {
				json.data = '上传失败';
				callback(json);
				return;
		...
		var str = JSON.stringify(data.Learning);
		client.query('update forumuser set Learning = ? where userid = ?', [str, data.userid], function(err, rows, fields) {
			if (err) {
				json.data = '上传失败';
				callback(json);
				return;
